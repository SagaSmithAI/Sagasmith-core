...
class ContinuityCommitService:
        ...
        *,
        ...
        with self.database.unit_of_work() as work:
            return self.commit_in_work(
                ...
                scene_progress_updates=scene_progress_updates,
        ...
        *,
        ...
        with self.database.operation(work) as session:
            ...
            if active_branch and has_state_mutation:
                revision_rows = (
                    StateMutationService(self.database).replace_in_work(
                        work,
                        campaign.id,
                        campaign_state=dict(campaign.state)
                        if campaign_state is None
                        else dict(campaign_state),
                        ...
                        branch_id=branch.id,
                        idempotency_key=idempotency_key,
                        idempotency_request_hash=request_hash(idempotency_write.payload)
            ...
            for progress_update in progress_updates:
                ...
                progress_results.append(
                    modules.set_scene_progress_in_work(
                        work, campaign_id=campaign.id, **dict(progress_update)
                    )
                )
            event_info = self.events.add_in_work(
                work,
                campaign,
                branch.id,
            ...
            fact_results = [
                self.apply_fact_in_work(work, campaign, branch.id, event_info.id, dict(item))
                for item in facts or []
            ]
            knowledge_results = [
                self.apply_knowledge_in_work(
                    work, campaign, branch.id, branch.head_snapshot_id, event_info.id, dict(item)
                )
            ...
            if snapshot is not None:
                ...
                snapshot_result = SnapshotService(self.database).create_in_work(
                    work,
                    campaign,
                    label=str(snapshot_data.get("label", "Continuity commit")),
                    recap=dict(snapshot_data["recap"])
            ...
            idempotency.remember_write_in_session(
                ...
                result=result,
                mutation_group_id=revision_rows[0].mutation_group_id if revision_rows else None,
            )
            return result

    def _apply_fact(self, session, *args, **kwargs):
        """0.2 consumer bridge; new applications pass UoW to apply_fact_in_work."""
        return self.apply_fact_in_work(self.database.work_for_session(session), *args, **kwargs)

    def _apply_knowledge(self, session, *args, **kwargs):
        """0.2 consumer bridge; new applications pass UoW to apply_knowledge_in_work."""
        return self.apply_knowledge_in_work(
            self.database.work_for_session(session), *args, **kwargs
        )

    def apply_fact_in_work(
        self,
        work: UnitOfWork,
        campaign: Campaign,
        branch_id: str,
        ...
        with self.database.operation(work) as session:
            ...
            if action == "revise":
                ...
                return self.facts.revise_in_work(
                    work,
                    memory,
                    branch_id,
                    content=content,
            ...
            if memory is not None:
                self.facts.require_existing_fact_identity(
                    memory,
                    {
                        field: data[field]
                ...
                if head is None:
                    ...
                    return self.facts.add_branch_revision_in_work(
                        work,
                        memory,
                        branch_id,
                        content=content,
    ...
    def apply_knowledge_in_work(
        ...
        with self.database.operation(work) as session:
            ...
            if action == "add":
                return self.knowledge.add_in_work(
                    work,
                    campaign,
                    branch_id,
                    head_snapshot_id,
            ...
            return self.knowledge.revise_in_work(
                work,
                knowledge,
                branch_id,
                head_snapshot_id,
                ...
                confidence=(
                    int(data["confidence"]) if data.get("confidence") is not None else None
                ),
                source_event_id=str(source_event_id),
